...
Use `deploy/truenas-upgrade.yaml`, not the older companion templates.
...
3. Edit the existing `pihole` app YAML. Paste the release upgrade YAML. It targets
   ...
   and `/mnt/M1.1/APPS/Super-Pi-Hole/{data,backups}`. Verify paths/case before saving.
   If your existing Super data lives elsewhere, preserve/mount that actual data
   directory instead of silently starting an empty one.
4. Set `SUPER_PIHOLE_PASSWORD` to a unique 16–256 character value. Set
...
The automatic backup reads the original directories but does not change them.
...
Stop the integrated app before restoring anything. Restore both directories
...
YAML with `pihole/pihole:2026.04.0` and its LAN web port 20720. Start only that
